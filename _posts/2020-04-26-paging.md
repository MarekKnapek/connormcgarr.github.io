...
This is an ideal example to show that a physical page of memory is actually _NOTHING MORE_ than a PFN multipiled by 0x1000 and an offset to the physical memory page! A PFN, as we can recall, is a reference to the base of the next paging structure. 
...
This is mainly because we were fetching the base address of a paging structure, which means it won't be offset from anything. A PFN.
...
Again, this was by no means meant to be an exhaustive and comprehensive "tell all" of memory paging. This article pretty much barely scratched the surface. However, understanding things like control bits and virtual memory and having that as prerequisite knowledge allows you to understand bypassing mitigations such as NX in kernel pool memory, or more ways of bypassing SMEP. The next post will go into bypassing SMEP and NX in the kernel by way of the prerequisite knowledge laid out here. 
